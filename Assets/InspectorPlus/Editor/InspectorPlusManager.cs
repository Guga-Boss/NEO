...
public class InspectorPlusManager : ScriptableObject
{
    ...
    public Texture2D NGUIImage;
    public Texture2D playMakerImage;
    ...
    void OnEnable()
    {
        ...
        if (arrowUp == null || arrowDown == null || NGUIImage == null || playMakerImage == null)
        {
            ...
            NGUIImage = (Texture2D)AssetDatabase.LoadAssetAtPath(Directory.GetParent(assetPath) + "/NGUI.png", typeof(Texture));
            playMakerImage = (Texture2D)AssetDatabase.LoadAssetAtPath(Directory.GetParent(assetPath) + "/PlayMaker.png", typeof(Texture));
        }
    }
    ...
    public List<string> GetGroup(string groupName, string filter = "" )
    {
        ...
        if (trackers.Count != 0)
        {
            ...
            if (!File.Exists(filePath + "/" + trackers[check].name + "InspectorPlus.cs"))
            {
                trackers.RemoveAt(check);
