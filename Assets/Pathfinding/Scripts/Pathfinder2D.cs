...
public class Pathfinder2D : MonoBehaviour 
{
    ...
    void Update()
    {
        ...
        while (queue.Count > 0 && timer < maxtime)
        {
            ...
            sw.Start();
            StartCoroutine(PathHandler(queue[0].startPos, queue[0].endPos, queue[0].storeRef, queue[0].Unit));
    ...
    public void Create2DMap( bool firsttime, Map m, bool canEnterArea )
    {
        ...
        if( m.CurrentArea != -1 )
        {
            minx = ( int ) m.CurArea.P1.x - 1;
            maxx = ( int ) m.CurArea.P2.x + 1;
            miny = ( int ) m.CurArea.P2.y - 1;
            maxy = ( int ) m.CurArea.P1.y + 1;
        ...
        for (int yy = miny; yy < maxy; yy++)
        {
            ...
            {
                float x = startX + (xx * Tilesize) + (Tilesize / 2); //Position from where we raycast - X
                float y = startY + (yy * Tilesize) + (Tilesize / 2); //Position from where we raycast - Z
                int ID = (yy * width) + xx; //ID we give to our Node!

				bool ok = m.Hero.CheckTerrainMove( new Vector2( -1, -1 ), new Vector2( xx, yy ), true, true, true, false, false );

				if(!canEnterArea )
				if( m.GetPosArea( new Vector2( xx, yy ) ) != -1 ) ok = false;

				if(!ok ) 
				{
					Map[ xx, yy ] = new Node( xx, yy, y, ID, x, 0, false );//Non walkable tile! 
				}
				else
				{
					Map[ xx, yy ] = new Node( xx, yy, y, ID, x, 0, true ); //walkable tile!
				}
            }
        }
    }
    ...
    public void InsertInQueue(Vector3 startPos, Vector3 endPos, Action<List<Vector3>> listMethod, Unit _unit)
    {
        QueuePath q = new QueuePath(startPos, endPos, listMethod, _unit);
        queue.Add(q);
    }
	...
	IEnumerator PathHandler( Vector3 startPos, Vector3 endPos, Action<List<Vector3>> listMethod, Unit _unit )
    {
        yield return StartCoroutine(SinglePath(startPos, endPos, listMethod, _unit));
    }

	IEnumerator SinglePath( Vector3 startPos, Vector3 endPos, Action<List<Vector3>> listMethod, Unit _unit )
    {
        FindPath(startPos, endPos, listMethod, _unit );
        yield return null;
    }

	public void FindPath( Vector3 startPos, Vector3 endPos, Action<List<Vector3>> listMethod, Unit _unit )
    {
        ...
        if (startNode != null)
        {
            ...
            while (!endLoop)
            {
				...
				MoveDiagonal = false;
				if( _unit.Control.MovementLevel >= 4 ) MoveDiagonal = true;

                if (MoveDiagonal)
                {
                    NeighbourCheck(_unit);
                }
                else
                {
					NonDiagonalNeighborCheck( _unit );
                }
            }
	...
	private void NeighbourCheck( Unit _unit )
    {
        int x = currentNode.x;
        int y = currentNode.y;
        ...
        for (int i = y - 1; i < y + 2; i++)
        {
            for (int j = x - 1; j < x + 2; j++)
            {
                ...
                if (i > -1 && i < Map.GetLength(1) && j > -1 && j < Map.GetLength(0))
                {
                    ...
                    if (i != y || j != x)
					if( MyMap.CheckArrowBlockFromTo( new Vector2( x, y ), new Vector2( j, i ), _unit ) == false )
                    {
	...
	private void NonDiagonalNeighborCheck( Unit _unit )
    {
        int x = currentNode.x;
        int y = currentNode.y;
