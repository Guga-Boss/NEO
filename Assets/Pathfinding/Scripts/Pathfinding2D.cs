...
public class Pathfinding2D : MonoBehaviour
{
    ...
    public void FindPath(Vector3 startPosition, Vector3 endPosition, Unit _unit)
    {
        Pathfinder2D.Instance.InsertInQueue(startPosition, endPosition, SetList, _unit);
    }

	public void FindJSPath( Vector3[] arr, Unit _unit )
    {
        if (arr.Length > 1)
        {
            Pathfinder2D.Instance.InsertInQueue(arr[0], arr[1], SetList, _unit);
        }
    }
