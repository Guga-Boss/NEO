...
public class tk2dTileMapEditor : Editor, ITileMapEditorHost
{
    ...
    void DrawTileDataSetupPanel()
    {
        ...
        if( selectedDataTile >= 0 && selectedDataTile < allTileInfos.Length )
        {
            ...
            tileInfo.Layer = ( ELayerType ) EditorGUILayout.EnumPopup( "Layer", tileInfo.Layer );                                  // guga add tk
            if( EditorGUI.EndChangeCheck() )
            {
                EditorUtility.SetDirty( tileMap ); // ou tileMapData, dependendo da estrutura
            }
            tileInfo.Blocked = EditorGUILayout.Toggle( "Blocked Tile", tileInfo.Blocked );

            if( Event.current.type == EventType.KeyDown )
            {
                KeyCode key = Event.current.keyCode;
                ELayerType newLayer = ELayerType.NONE;
                bool setLayer = true;

                switch( key )
                {
                    case KeyCode.Alpha1: newLayer = ELayerType.TERRAIN; break;                                                     // sets layers by num keys
                    case KeyCode.Alpha2: newLayer = ELayerType.GAIA; break;
                    case KeyCode.Alpha3: newLayer = ELayerType.GAIA2; break;
                    case KeyCode.Alpha4: newLayer = ELayerType.MONSTER; break;
                    case KeyCode.Alpha5: newLayer = ELayerType.MODIFIER; break;
                    case KeyCode.Alpha6: newLayer = ELayerType.GRID; break;
                    case KeyCode.Alpha7: newLayer = ELayerType.DECOR; break;
                    case KeyCode.Alpha8: newLayer = ELayerType.AREAS; break;
                    case KeyCode.Alpha9: newLayer = ELayerType.RAFT; break;
                    case KeyCode.B: setLayer = false; tileInfo.Blocked = true; break;
                    case KeyCode.U: setLayer = false; tileInfo.Blocked = false; break;
                    default: setLayer = false; break;                                                                               
                }

                if( setLayer )
                {
                    tileInfo.Layer = newLayer;
                    GUI.changed = true;
                    Event.current.Use(); // Consome a tecla

                    Debug.Log( "Layer definida via teclado: " + newLayer.ToString() );                                                // fim do meu codigo
                }
            }
        }
    ...
    void DrawPaintPanel()
    {
        ...
        ///////////////////////////////////////////////////////////
        // guga add tk
        int tl = tk2dRuntime.TileMap.BuilderUtil.GetTileFromRawTile( activeBrush.tiles[ 0 ].spriteId );

        UpdateAutoSelection( tl );

        if( Map.I == null ) 
            Map.I = GameObject.Find( "----------------Map" ).GetComponent<Map>();
        ELayerType layer = Map.GetTileLayer( ( ETileType ) tl );
        if( ( layer != ELayerType.DECOR && layer != ELayerType.DECOR2 ) ||
            ( tl == ( int ) ETileType.BOOL_TOOGLE_1 || tl == ( int ) ETileType.BOOL_TOOGLE_2 ) )
        {
            editorData.layer = ( int ) layer;
        }
        else
        {
            if( editorData.layer != ( int ) ELayerType.DECOR &&
                editorData.layer != ( int ) ELayerType.DECOR2 )
                editorData.layer = ( int ) ELayerType.DECOR2;
        }

        if( editorData.layer == -1 ) editorData.layer = 0;

        editorData.showLayer = EditorGUILayout.Foldout( editorData.showLayer, "Layer" );
        if( editorData.showLayer )
            if( tileMap.name == "Areas Template Tilemap" )
            {
                EModType mdd = Mod.GetMod( ( ETileType ) tl );
                if( mdd != EModType.NONE )                                               // select auto mod by clicking over the palette
                    MapSaver.Get().AutoMod = mdd;

                if( Map.GetTileID( ( ETileType ) tl ) == ETileType.ORIENTATION )         // select auto ori by clicking over the palette
                    MapSaver.Get().AutoOri = ( EOrientatorType ) tl;

                int key = Mod.GetKeyNum( ( ETileType ) tl );
                if( key > -1 ) MapSaver.Get().AutoOri2 = ( EOri2Template ) key;

                key = Mod.GetOri3Num( ( ETileType ) tl );
                if( key > -1 ) MapSaver.Get().AutoOri3 = ( EOri3Template ) key;

                EModType md = ( EModType ) EditorGUILayout.EnumPopup( new GUIContent( "Auto MOD", "Auto MOD" ), MapSaver.Get().AutoMod );   // auto mod popup
                MapSaver.Get().AutoMod = md;

                EOrientatorType or = ( EOrientatorType ) EditorGUILayout.EnumPopup( new GUIContent( "Auto ORI", "Auto ORI" ), MapSaver.Get().AutoOri );   // auto ori popup
                MapSaver.Get().AutoOri = or;

                EOri2Template or2 = ( EOri2Template ) EditorGUILayout.EnumPopup( new GUIContent( "Auto ORI2", "Auto ORI2" ), MapSaver.Get().AutoOri2 );   // auto Ori2
                MapSaver.Get().AutoOri2 = or2;

                EOri3Template or3 = ( EOri3Template ) EditorGUILayout.EnumPopup( new GUIContent( "Auto ORI3", "Auto ORI3" ), MapSaver.Get().AutoOri3 );   // auto Ori3
                MapSaver.Get().AutoOri3 = or3;
        ...
        if( !showLoadSection && !showSaveSection )
        {
            ...
            if( editorData.showPalette )
            {
                ...
                if( activeBrush.tiles.Length == 1 )
                {
                    ...
                    ETileType tp = Map.GetTileID( ( ETileType ) tile );
                    string nm = " " + tp;                   
                    string add = "";
                    int key = Mod.GetKeyNum( ( ETileType ) tl );
                    if( key > -1 ) add = " ( " + key + " ) ";
                    ...
                    if( tile >= 0 && tile < SpriteCollection.spriteDefinitions.Length )
                        selectionDesc = SpriteCollection.spriteDefinitions[ tile ].name + "                 Name: " + nm + add + "              " + " -NEO is a GREAT SUCCESS!-";
                }
                ...
                // AUTO SCROLL PALETTE COM MOUSE NAS BORDAS (tipo Age of Empires)    // guga add tk
                //float autoScrollSpeed = 15;
                //Vector2 mousePos = Event.current.mousePosition;
                //if( mousePos.y <= 675 )
                //    paletteScrollPos.y -= autoScrollSpeed;
                //else if( mousePos.y >= 1105 )
                //    paletteScrollPos.y += autoScrollSpeed;
                //if( mousePos.x <= 40 )
                //    paletteScrollPos.x -= autoScrollSpeed;
                //else if( mousePos.x >= 534 )
                //    paletteScrollPos.x += autoScrollSpeed;
                Repaint();

                // Scroll vertical + horizontal
                paletteScrollPos = EditorGUILayout.BeginScrollView(
                    paletteScrollPos,
                    GUILayout.Height( Mathf.Min( innerRect.height + 40.0f, 500f ) ) // limita altura máxima
                );
    ...
    public override void OnInspectorGUI()
    {
        ...
        {
            GUILayout.BeginHorizontal();
            if( GUILayout.Button( "Edit Cube Tilemap", GUILayout.Height( 40 ) ) )
            {
        ...
        EMapTemplate lt = EMapTemplate.__CHOOSE__;
        lt = ( EMapTemplate ) EditorGUILayout.EnumPopup( new GUIContent( "Load Template", "Load Template" ), lt );   // guga add tk
        if( lt != EMapTemplate.__CHOOSE__ )
        {

            GameObject go = GameObject.Find( "Navigation TileMap" );
            tk2dTileMap hh = go.GetComponent<tk2dTileMap>();

            MapSaver.Get().MapTemplate = lt;
            bool res = MapSaver.Get().Load();
            if( !res )
                Debug.Log( "Template: " + lt + " Could not be found." );
            MapSaver.Get().MapTemplate = EMapTemplate.__CHOOSE__;
        }

        EMapTemplate st = EMapTemplate.__CHOOSE__;
        st = ( EMapTemplate ) EditorGUILayout.EnumPopup( new GUIContent( "Save Template", "Save Template" ), st );
        if( st != EMapTemplate.__CHOOSE__ )
        {
            MapSaver.Get().MapTemplate = st;
            MapSaver.Get().Save();
            MapSaver.Get().MapTemplate = EMapTemplate.__CHOOSE__;
        }
