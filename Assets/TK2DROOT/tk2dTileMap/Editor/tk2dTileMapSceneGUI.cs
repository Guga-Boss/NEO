...
public class tk2dTileMapSceneGUI
{
	...
	void DrawTileMapRectCursor(int x0, int y0, int x1, int y1)
	{
        ...
        if( Helper.I == null ) Helper.I = GameObject.FindObjectOfType<Helper>();

        Helper.I.TileMapCord = "" + ( p0.x + .5f ) + " " + ( p0.y + .5f );   // guga add tk
        Helper.I.TileMapCordVector = new Vector2( ( p0.x + .5f ), ( p0.y + .5f ) );
	}
	...
	{	
        ...
        if( tk2dTileMapToolbar.mainMode == tk2dTileMapToolbar.MainMode.Pen )  // guga add tk pen drawing
        {
    ...
    private void UpdateAutoTiling()                                             // guga add tk autotiling for decor
    {
        ...
        for( int x = 0; x < tileMap.width; x++ )                                     // idea: fazer um generico para linear  e vert big e small tipo checksmall()
        {
            CheckTile( 3976, x, y, -1, 0, 3975 );  // linear vine
            CheckTile( 3976, x, y, +1, 0, 3977 );
            CheckTile( 3470, x, y, 0, +1, 3342 );
            CheckTile( 3470, x, y, 0, -1, 3598 );
            CheckTile( 3975, x, y, +1, 0, 3977 );
            CheckTile( 3977, x, y, -1, 0, 3975 );
            CheckTile( 3342, x, y, 0, -1, 3598 );
            CheckTile( 3598, x, y, 0, +1, 3342 );
            //CheckTile( Util.Conv( 422 ), x, y, -1, 0, Util.Conv( 421 ) );  // linear H big vine left
            //CheckTile( Util.Conv( 438 ), x, y, -1, 0, Util.Conv( 437 ) );  // 
            //CheckTile( Util.Conv( 454 ), x, y, -1, 0, Util.Conv( 453 ) );  // 
            //CheckTile( Util.Conv( 423 ), x, y, -1, 0, Util.Conv( 421 ) );  // linear H big vine 2
            //CheckTile( Util.Conv( 439 ), x, y, -1, 0, Util.Conv( 437 ) );  // 
            //CheckTile( Util.Conv( 455 ), x, y, -1, 0, Util.Conv( 453 ) );  // 
            //CheckTile( Util.Conv( 422 ), x, y, +1, 0, Util.Conv( 424 ) );  // linear H big vine right
            //CheckTile( Util.Conv( 438 ), x, y, +1, 0, Util.Conv( 440 ) );  // 
            //CheckTile( Util.Conv( 454 ), x, y, +1, 0, Util.Conv( 456 ) );  // 
            //CheckTile( Util.Conv( 423 ), x, y, +1, 0, Util.Conv( 424 ) );  // linear H big vine 2 up
            //CheckTile( Util.Conv( 439 ), x, y, +1, 0, Util.Conv( 440 ) );  // 
            //CheckTile( Util.Conv( 455 ), x, y, +1, 0, Util.Conv( 456 ) );  //
            //CheckTile( Util.Conv( 373 ), x, y, 0, +1, Util.Conv( 357 ) );  // linear V big vine up
            //CheckTile( Util.Conv( 374 ), x, y, 0, +1, Util.Conv( 358 ) );  // 
            //CheckTile( Util.Conv( 375 ), x, y, 0, +1, Util.Conv( 359 ) );  // 
            //CheckTile( Util.Conv( 389 ), x, y, 0, +1, Util.Conv( 357 ) );  // linear V big vine down 2
            //CheckTile( Util.Conv( 390 ), x, y, 0, +1, Util.Conv( 358 ) );  // 
            //CheckTile( Util.Conv( 391 ), x, y, 0, +1, Util.Conv( 359 ) );  // 
            //CheckTile( Util.Conv( 373 ), x, y, 0, -1, Util.Conv( 405 ) );  // linear V big down left
            //CheckTile( Util.Conv( 374 ), x, y, 0, -1, Util.Conv( 406 ) );  // 
            //CheckTile( Util.Conv( 375 ), x, y, 0, -1, Util.Conv( 407 ) );  // 
            //CheckTile( Util.Conv( 389 ), x, y, 0, -1, Util.Conv( 405 ) );  // linear V big down left 2
            //CheckTile( Util.Conv( 390 ), x, y, 0, -1, Util.Conv( 406 ) );  // 
            //CheckTile( Util.Conv( 391 ), x, y, 0, -1, Util.Conv( 407 ) );  // 
        }
    }

    private void CheckTile( int srctile, int x, int y, int difx, int dify, int tile )
    {
        int tileatpos = tileMap.GetTile( x, y, ( int ) ELayerType.DECOR2 );
        if( srctile == tileatpos )
        {
            if( x + difx < tileMap.width )
            if( x + difx >= 0 )
            if( y + dify < tileMap.height )
            if( y + dify >= 0 )
            if( tileMap.GetTile( x + difx, y + dify, ( int ) ELayerType.DECOR2 ) == -1 )
                tileMap.SetTile( x + difx, y + dify, ( int ) ELayerType.DECOR2, tile );
        }
    }
	...
	public void PencilDrag() {
		pencilDragActive = true;

        if( tk2dTileMapToolbar.mainMode == tk2dTileMapToolbar.MainMode.Pen )                                          // guga add tk pen drawing
        {
            ELayerType layer = Map.GetTileLayer( ( ETileType ) editorData.activeBrush.tiles[ 0 ].spriteId );
            SplatTile( cursorX, cursorY, ( int ) layer, editorData.activeBrush.tiles[ 0 ].spriteId );    
        }
	...
	public void OnSceneGUI()
	{
		...
		{
			bool pickupKeyDown = (Application.platform == RuntimePlatform.OSXEditor)?Event.current.control:Event.current.shift;
    ...
    public void UpdateTileClone(  )            // guga add tk: clones the tile by right click
    {
        Map.I = GameObject.Find( "----------------Map" ).GetComponent<Map>();
        int mn = tileMap.GetTile( cursorX, cursorY, ( int ) ELayerType.MONSTER );
        int ori2 = tileMap.GetTile( cursorX, cursorY, ( int ) ELayerType.GAIA2 );
        int ga = tileMap.GetTile( cursorX, cursorY, ( int ) ELayerType.GAIA );
        if( mn != -1 ) 
        {
            SetMyBrush( mn, ELayerType.MONSTER );
        }
        else
            if( ori2 == ( int ) ETileType.ITEM )
        {
            SetMyBrush( mn, ELayerType.GAIA2 );
        }
        else
        if( ga != -1 )
        {
            SetMyBrush( ga, ELayerType.GAIA );
        }

        MapSaver.Get().AutoMod = EModType.NONE;
        MapSaver.Get().AutoOri = EOrientatorType.NONE;
        MapSaver.Get().AutoOri2 = EOri2Template.NONE;
        MapSaver.Get().AutoOri3 = EOri3Template.NONE;

        int mod = tileMap.GetTile( cursorX, cursorY, ( int ) ELayerType.MODIFIER );
        mod = ( int ) Mod.GetMod( ( ETileType ) mod );
        if( mod != -1 )
            MapSaver.Get().AutoMod = ( EModType ) mod;

        int ori = tileMap.GetTile( cursorX, cursorY, ( int ) ELayerType.AREAS );
        if( Map.GetTileID( ( ETileType ) ori ) == ETileType.ORIENTATION )
            MapSaver.Get().AutoOri = ( EOrientatorType ) ori;
        if( Map.GetTileID( ( ETileType ) ori2 ) == ETileType.DOOR_OPENER )
            MapSaver.Get().AutoOri2 = ( EOri2Template ) ori2 - ( int ) ETileType.DOOR_OPENER;
        if( Map.GetTileID( ( ETileType ) ori2 ) == ETileType.DOOR_SWITCHER )
            MapSaver.Get().AutoOri2 = ( EOri2Template ) ori2 - ( int ) ETileType.DOOR_SWITCHER + 12;
        if( Map.GetTileID( ( ETileType ) ori2 ) == ETileType.DOOR_CLOSER )
            MapSaver.Get().AutoOri2 = ( EOri2Template ) ori2 - ( int ) ETileType.DOOR_CLOSER + 24;
        if( Map.GetTileID( ( ETileType ) ori2 ) == ETileType.DOOR_KNOB )
            MapSaver.Get().AutoOri2 = ( EOri2Template ) ori2 - ( int ) ETileType.DOOR_KNOB + 36;
        int ori3 = tileMap.GetTile( cursorX, cursorY, ( int ) ELayerType.RAFT );
        if( Map.GetTileID( ( ETileType ) ori3 ) == ETileType.ORI3 )
            MapSaver.Get().AutoOri3 = ( EOri3Template ) ori3 - ( int ) ETileType.ORI3;
        if( ( int ) ori3 >= 1408 && ( int ) ori3 < 1408 + Map.TotOri3 )
            MapSaver.Get().AutoOri3 = ( EOri3Template ) ori3 - 1408 + Map.TotOri3;
    }

    private void SetMyBrush( int tl, ELayerType layer )             // guga add tk
    {
        editorData.activeBrush.multiSelectTiles[ 0 ] = tl;
        editorData.activeBrush.tiles[ 0 ].spriteId = tl;
        editorData.activeBrush.tiles[ 0 ].layer = ( int ) layer;
        editorData.layer = ( int ) layer;
    }
	...
	void SplatTile(int x, int y, int layerId, int spriteId)
	{
		if (scratchpadGUI.workingHere) {
				...
				layerId >= 0 && layerId < tileMap.data.NumLayers)
			{
                ...
                // guga add tk autolayer for decor

                MapSaver ms = MapSaver.Get();
                if( ms && ms.AutoDecorLayer  )
                if( layerId == ( int ) ELayerType.DECOR || layerId == ( int ) ELayerType.DECOR2 )
                {
                    int id = layerId;
                    int tl1 = tileMap.GetTile( x, y, ( int ) ELayerType.DECOR );
                    int tl2 = tileMap.GetTile( x, y, ( int ) ELayerType.DECOR2 );
                    if( tl1 != -1 && tl2 != -1 ) return;
                    if( tl1 == -1 && tl2 == -1 ) { } else
                    {
                        if( tl1 != -1 ) id = ( int ) ELayerType.DECOR2;
                        if( tl2 != -1 ) id = ( int ) ELayerType.DECOR;
                    }
                    if( x == 0  || y == 0 ||
                        x == 28 || y == 28 ) 
                        id = ( int ) ELayerType.DECOR2;
                    layer = tileMap.Layers[ id ];
                }
                ...
                // ------------------------------------------------------------------------- guga add tk auto add mod

                bool ok = true;
                ETileType sp = Map.GetTileID( ( ETileType ) spriteId );
                if( sp == ETileType.CHECKPOINT ) ok = false;
                if( sp == ETileType.ORB ) ok = false;
                if( sp == ETileType.CHECKPOINT ) ok = false;
                if( sp == ETileType.SAVEGAME ) ok = false;
                if( sp == ETileType.TRAP ) ok = false;
                if( sp == ETileType.FREETRAP ) ok = false;
                if( sp == ETileType.SCROLL ) ok = false;
                //if( sp == ETileType.ARROW ) ok = false;
                if( sp == ETileType.FISH ) ok = false;
                if( sp == ETileType.DOOR_OPENER ) ok = false;
                if( sp == ETileType.DOOR_SWITCHER ) ok = false;
                if( sp == ETileType.DOOR_CLOSER ) ok = false;
                if( sp == ETileType.DOOR_KNOB ) ok = false;

                if( ok )
                if( layerId == ( int ) ELayerType.GAIA2 ||
                    layerId == ( int ) ELayerType.MONSTER )
                {
                    if( ms && ms.AutoMod != EModType.NONE )
                    {
                        ETileType tl = Mod.GetTileIDforMod( ms.AutoMod );
                        tileMap.SetTile( x, y, ( int ) ELayerType.MODIFIER, ( int ) tl );
                    }
                    // ------------------------------------------------------------------------- guga add tk auto add ori
                    if( ms && ms.AutoOri != EOrientatorType.NONE )
                    {
                        ETileType tl = ( ETileType ) ms.AutoOri;
                        tileMap.SetTile( x, y, ( int ) ELayerType.AREAS, ( int ) tl );
                    }

                    if( ms && ms.AutoOri2 != EOri2Template.NONE )
                    {
                        int res = -1;
                        ETileType tl = ( ETileType ) ms.AutoOri2 + ( int ) ETileType.DOOR_OPENER;
                        if( Map.GetTileID( tl ) == ETileType.DOOR_OPENER ) res = ( int ) tl;
                        tl = ( ETileType ) ms.AutoOri2 + ( int ) ETileType.DOOR_SWITCHER - 12;
                        if( Map.GetTileID( tl ) == ETileType.DOOR_SWITCHER ) res = ( int ) tl;
                        tl = ( ETileType ) ms.AutoOri2 + ( int ) ETileType.DOOR_CLOSER - 24;
                        if( Map.GetTileID( tl ) == ETileType.DOOR_CLOSER ) res = ( int ) tl;
                        tl = ( ETileType ) ms.AutoOri2 + ( int ) ETileType.DOOR_KNOB - 36;
                        if( Map.GetTileID( tl ) == ETileType.DOOR_KNOB ) res = ( int ) tl;
                        if( res >= 0 )
                            tileMap.SetTile( x, y, ( int ) ELayerType.GAIA2, res );
                    }

                    if( ms && ms.AutoOri3 != EOri3Template.NONE )
                    {
                        int res = -1;
                        int tl = ( ( int ) ms.AutoOri3 + ( int ) ETileType.ORI3 );
                        if( ( int ) ms.AutoOri3 <= Map.TotOri3 )
                        if( Map.GetTileID( ( ETileType ) tl ) == ETileType.ORI3 ) res = ( int ) tl;
                        if( res == -1 )
                        {
                            tl = ( ( int ) ms.AutoOri3 + ( int ) 1408 - 10 );
                            if( Map.GetTileID( ( ETileType ) tl ) == ETileType.ORI3 ) res = ( int ) tl;
                        }
                        if( res >= 0 )
                            tileMap.SetTile( x, y, ( int ) ELayerType.RAFT, res );
                    }
                }
			}
		}
