...
public static class tk2dTileMapToolbar
{
	...
	public static void ToolbarWindow() {
        ...
        if( HiliteBtn( tk2dEditorSkin.GetTexture( "icon_pencil" ), ( mainMode == MainMode.Pen ), tk2dPreferences.inst.tileMapToolColor_brush, "Draw" ) ) mainMode = MainMode.Pen;
        GUILayout.Space( 10 );
        ...
        EditorGUILayout.TextField( "", "Tile Cord: " + Helper.I.TileMapCord, style );  /////////////////////////  Guga add tk 

        MapSaver ms = MapSaver.Get();

        if( Helper.I.TileMapCordVector.x == 0  ||
            Helper.I.TileMapCordVector.y == 0  ||
            Helper.I.TileMapCordVector.x == 28 ||
            Helper.I.TileMapCordVector.y == 28 )
        if( Selection.activeGameObject.name == "Areas Template Tilemap" )
        {
            EModType md = ms.AutoMod;
            if( md != EModType.NONE )
            {
                string txt = Helper.I.GetOriListText();
                txt = Util.GetTextBetween( txt, "BEGIN_MOD_" + ( int ) ( ms.AutoMod + 1 ), "END_MOD_" + ( int ) ( ms.AutoMod + 1 ) );
                EditorGUILayout.TextField( "", "Mod Description: --- " + ms.AutoMod + " ---\n\n" + txt, style );  // Guga add tk   Shows MOD description List
            }
            else
            {
                string ls = Helper.I.GetModDescriptionText();
                if( ls != "" )
                {
                    Helper.I.ModDescriptionList = ls;
                    EditorGUILayout.TextField( "", "Mod Description:\n" + ls, style );  // Guga add tk   Shows MOD description List
                }
            }
        }
	}
