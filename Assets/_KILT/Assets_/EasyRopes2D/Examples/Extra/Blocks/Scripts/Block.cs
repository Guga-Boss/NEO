...
public abstract class Block : MonoBehaviour {
	...
	public event Delegates.EventHandler<DamageEventArgs> OnDamaged;
	public event Delegates.EventHandler OnDie;
	public event Delegates.EventHandler OnDestroyedByUser;
	...
	{
		bool v_canBeClicked = Block.GlobalPressOnly? false : AcceptUserClick;
	...
	{
		Camera v_camera = CameraManager.GetCameraThatDrawLayer(gameObject.layer);
		if(v_camera != null)
		{
	...
	{
		return !KiltUtils.IsOutOfScreen(gameObject, false, true, false);
		/*Camera v_camera = CameraManager.GetCameraThatDrawLayer(gameObject.layer);
		if(v_camera != null)
		{
			Vector2 v_viewPortPos = v_camera.WorldToViewportPoint(transform.position);
			if(v_viewPortPos.x >=-0.1 && v_viewPortPos.x <=1.1 && v_viewPortPos.y >=-0.1 && v_viewPortPos.y <=1.1)
			{
				return true;
			}
		}
	...
	protected virtual void OnCollisionEnter2D(Collision2D p_collision) 
	{
		...
		{
			Block v_otherBlock = v_otherCollidedObject.GetComponent<Block>();
...
public class DamageEventArgs
{
	...
	public Block Sender {get {return m_sender;} set{m_sender = value;}}

	#endregion

	public DamageEventArgs(Block p_sender , float p_damage)
	{
		m_damage = p_damage;
		m_sender = p_sender;
	}
