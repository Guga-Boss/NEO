...
public class Rope2D : MonoBehaviour 
{
	...
	public Delegates.EventHandler<Rope2D> OnRopeDestroyed;
	public Delegates.EventHandler<Rope2D> OnRopeCreated;
	public Delegates.EventHandler<Rope2D, int> OnRopeBreak; //Parameter = Node Index
	public Delegates.EventHandler<Rope2D, int> OnRopeCut; //Parameter = Node Index
	public Delegates.EventHandler<Rope2D, float> OnRopeTensioned; // Float Value will be A deltaTensionedValue(value 1 == Rope Will Break)
	
	#endregion
	
	#region Static Properties
	
	public static Rope2D[] _allRopesInScene = new Rope2D[0];
	
	public static Rope2D[] AllRopesInScene 
	{
		get
		{
			return _allRopesInScene;
		}
	}
	...
	[SerializeField, PropertyBackingFieldAttribute]
    public Sprite m_nodeSprite = null;
	[SerializeField, PropertyBackingFieldAttribute]
	...
	//Misc
	/* when StableFirstAndLastNodes mode is enable, the first and last node will be plugged by the center of mass instead of the borders.
	 * increasing rope stability. Only disable this property when UseLineRenderer = false and/or your PlugguedObject 
	 * is smaller then one node of this rope */
	[SerializeField, PropertyBackingFieldAttribute]
	bool m_stableFirstAndLastNodes = true;
	[SerializeField, PropertyBackingFieldAttribute]
	...
	//Destroy Properties
	public RopeBreakActionEnum RopeBreakAction {get{return m_ropeBreakAction;} set{m_ropeBreakAction = value;}}
	public float DestroyTime {get{return m_destroyTime;} set{m_destroyTime = value;}}
	public bool LowMassVariationWhenBroken {get{return m_lowMassVariationWhenBroken;} set{m_lowMassVariationWhenBroken = value;}}
	...
	public virtual void DrawDebugGizmos()
	{
		...
		try
		{
			...
			if(ObjectA != null && ObjectB != null)
			{
				...
				Vector2 v_direction = VectorHelper.Direction(v_inicialPosition, v_finalPosition);
	...
	public void DrawDebugLineInGameScene(bool p_force = false)
	{
		#if UNITY_EDITOR
		if(_isDrawingGizmos)
			_isDrawingGizmos = false;
		else if((!Application.isPlaying || p_force) && ObjectA != null && ObjectB != null)
		{
			Vector2 v_inicialPosition = ObjectA.transform.position;
			Vector2 v_finalPosition = ObjectB.transform.position;
			DebugDraw.DrawLine2D(v_inicialPosition, v_finalPosition, new Color(1, 0.5f, 0.5f), this.gameObject.layer);
		}
		#endif
	}
	...
	{
		return GetTensionDelta() > 0? true : false;
	}
	...
	protected void CheckTension()
	{
		if(ObjectA != null && ObjectB != null && Application.isPlaying)
		{
			...
			if(v_ropeTensionDelta > 0 && !IsRopeBroken())
			{
				if(v_distance > _nonTensionedRopeSize * 1.1)
				{
			...
			if(_tensionHelper != null && _tensionHelper.enabled != IsRopeTensioned())
			{
				_tensionHelper.enabled = IsRopeTensioned();
				if(!_tensionHelper.enabled)
					ResetDeltaTimeToCheckIfRopeNeedBreak(0.2f); //Prevent rope to break when returning
			}
	...
	protected void InsertTensionHelper()
	{
		if(!IsRopeBroken() && Application.isPlaying)
		{
			...
			{
				float v_distanceCorrector = 0.9f;
				if(TensionHelperAddOption == TensionHelperAddOptionEnum.AddInPluggedObjects)
				{
					v_distanceCorrector = IsSpringNode? 0.9f : 0.9f; //Distance corrector is different to Spring and Distance Joints in this mode
	...
	protected void DestroyTensionHelper()
	{
		if(_tensionHelper != null)
		{
			KiltUtils.Destroy(_tensionHelper);
			_tensionHelper = null;
		}
	}
	...
	protected void CorrectNodeDistanceAndRotation(bool p_correctFirstAndLast, bool p_force = false)
	{
		if((!IsRopeBroken() || p_force) && ObjectA != null && ObjectB != null)
		{
			...
			for(int i=0; i< Nodes.Count; i++)
			{
				...
				{
					//Correct Rotation
					Vector3 v_direction = VectorHelper.Direction(ObjectA.transform.position, ObjectB.transform.position);
		...
		if(ObjectA != null && ObjectB != null && AmountOfNodes > 0)
		{
			...
			else
			{
				...
				while(Nodes.Count > v_trueAmount && Nodes.Count > 0)
				{
					...
					KiltUtils.DestroyImmediate(v_object);
				...
				if(v_newNode != null && !Nodes.Contains(v_newNode))
				{
					if(i < Nodes.Count)
					{
						if(Nodes[i] != null && Nodes[i] != v_newNode)
							KiltUtils.DestroyImmediate(Nodes[i]);
						Nodes[i] = v_newNode;
					}
					else
						Nodes.Add(v_newNode);
	...
	public virtual GameObject CreateNode(GameObject p_modelNode, Vector2 p_position, Quaternion p_rotation, GameObject p_connectedNode, GameObject p_chunk, Sprite p_sprite = null, string p_name = "Node", float p_nodeMass = 0.05f, bool p_generateHingeJoint = true, bool p_isSpringJoint = true, bool p_generateDistanceOrSpringJoint = true)
	{
		...
		//GameObject
		GameObject v_newNode = p_modelNode != null? (KiltUtils.IsPrefab(p_modelNode)? KiltUtils.Instantiate(p_modelNode, false) : p_modelNode) : new GameObject(p_name);
		...
		if(v_renderer == null)
		{
			...
			if(NodeMaterial != null)
				v_renderer.material = NodeMaterial;
		}
		...
		if(p_generateDistanceOrSpringJoint)
			PlugDistanceOrSpringJoint(p_connectedNode, v_newNode, GetNodeDistance(false), p_isSpringJoint);
						...
						foreach(GameObject v_chunk in v_shuffledChunks)
						{
							   ...
							   v_lastIndex >=0 && v_lastIndex < Nodes.Count)
							{
								if(Nodes[v_firstIndex] != null && Nodes[v_lastIndex] != null)
								{
									...
									{
										BreakNode(v_shuffledNodes[KiltUtils.RandomRange(v_firstIndex,v_lastIndex+1)]);
	...
	protected void BreakNode(GameObject p_node)
	{
		...
		{
			BreakNode(Nodes.ObjectIndex(p_node));
		}
	}
	...
	protected virtual void BreakNode(int p_nodeIndex)
	{
		if(p_nodeIndex >= 0 && p_nodeIndex < Nodes.Count)
		{
			...
			KiltUtils.DestroyImmediate(v_node);
	...
	protected virtual void StartRopeBreakAction()
	{
		...
		{
			AlphaScheduler v_scheduler = GetComponent<AlphaScheduler>();
			if(v_scheduler == null)
			{
				v_scheduler = gameObject.AddComponent<AlphaScheduler>();
				v_scheduler.IsPing = false;
				v_scheduler.PropagateToChildrens = true;
				v_scheduler.CancelOnClick = false;
				v_scheduler.IgnoreTimeScale = false;
				v_scheduler.TimerStyle = TimerStyle.OneTimeOnly;
				v_scheduler.DestroyOptionWhenStop = WhenStopDestroyEnum.DontDestroy;
				v_scheduler.EventReceiverObject = this.gameObject;
				v_scheduler.CallFunctionWithNameWhenStop = "RemoveObject";
				v_scheduler.InitialDelay = 0.4f;
				v_scheduler.MaxTime = DestroyTime;
				v_scheduler.StartTimer(true, true);
			}
		}
	}
	...
	public virtual bool CutNode(GameObject p_node, bool p_force = false)
	{
		...
		{
			int v_nodeIndex = Nodes.ObjectIndex(p_node);
			BreakNode(p_node);
			if(OnRopeCut != null)
	...
	public void SendMessageToAllIndirectObjects(string p_functionToCall = "IndirectRopeDestroyed")
	{
		if(Application.isPlaying)
		{
			...
			List<RopesAttached> v_attachedComponents = KiltUtils.GetListOfComponentsFromListOfGameObjects<RopesAttached>(GetPluggedObjectsInRope());
		...
		if(p_chunk != null)
		{
			...
			if(v_firstIndex >= 0 && v_firstIndex < Nodes.Count && v_lastIndex >= 0 && v_lastIndex < Nodes.Count)
			{
				for(int i = v_firstIndex; i < v_lastIndex+1; i++)
				{
					...
					v_chunkMass += KiltUtils.GetObjectMass(v_node,true,p_multiplyByGravityScale);
				}
			}
		}
		return v_chunkMass;
	...
	public void UpdateNodeBounds()
	{
		...
		if(NodeSprite != null)
		{
			...
			if((Application.isPlaying && !Application.isEditor) || (Application.isEditor && !KiltUtils.IsPrefab(this.gameObject)) )
		...
		KiltUtils.DestroyImmediate(v_boundsObject);
	}
    ...
    {
        // ADD THIS: Stop all node coroutines before clearing
        foreach( GameObject node in Nodes )
        {
            if( node != null )
            {
                Node2D nodeComp = node.GetComponent<Node2D>();
                if( nodeComp != null )
                {
                    // This will stop the collision coroutine
                    nodeComp.ApplyIgnoreRopeCollision(); // This now stops previous coroutines
                }
            }
        }
	...
	{
		Rope2D[] v_ropes = KiltUtils.FindAllComponentsOfType<Rope2D>();
		foreach(Rope2D v_rope in v_ropes)
		{
