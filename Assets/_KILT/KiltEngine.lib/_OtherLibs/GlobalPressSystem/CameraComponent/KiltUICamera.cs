...
/// <summary>
/// Extension of UICamera. This class works even if NGUI is not project. Camera extension works with GraphicRaycast(Cannot be Disabled), Raycast2D and Raycast3D
/// 
...
#if NGUI_DLL
public class KiltUICamera : UICamera
	#else
	public class KiltUICamera : MonoBehaviour
	#endif
{
	...
	public static event EventHandler<bool> OnGlobalPress;
	public static event EventHandler<Vector2> OnGlobalDrag;
	public static event EventHandler<GameObject> OnGlobalDrop;
	...
	static public KiltUICamera current = null;
		...
		set
		{
			if (mSel != value)
			{
				if (mSel != null)
				{
					...
					#else
					KiltUICamera uicam = FindCameraForLayer(mSel.layer);
					#endif
	...
	static public Camera mainCamera
	{
		get
		{
			...
			#else
			KiltUICamera mouse = eventHandler;
			#endif
	...
	#else
	static public KiltUICamera eventHandler
	{
		get
		{
			for (int i = 0; i < UICamList.Count; ++i)
	...
	#region Private Variables from UICamera Getters/Setters
	
	#if NGUI_DLL
	// List of all active cameras in the scene
	static System.Reflection.FieldInfo _listInfo = null;
	protected static List<UICamera> UICamList
	{
		get
		{
			#if NGUI_DLL
			...
			if(_listInfo == null)
				_listInfo = v_type.GetField("mList", System.Reflection.BindingFlags.NonPublic| System.Reflection.BindingFlags.Static);
			System.Reflection.FieldInfo v_fieldInfo = _listInfo;
			object v_currentValue = v_fieldInfo.GetValue(null);
			return v_currentValue as List<UICamera>;
			#else
			return mList;
			#endif
		}
	...
	#else
	static List<KiltUICamera> m_uiCamList = new List<KiltUICamera>();
	protected static List<KiltUICamera> UICamList
	{
		get
		{
			if(m_uiCamList == null)
				m_uiCamList = new List<KiltUICamera>();
			return m_uiCamList;
		}
		set
		{
			if(m_uiCamList == value)
				return;
			m_uiCamList = value;
		}
	}
	...
	#else
	static int CompareFunc (KiltUICamera a, KiltUICamera b)
	{
		if (a.cachedCamera.depth < b.cachedCamera.depth) return 1;
		if (a.cachedCamera.depth > b.cachedCamera.depth) return -1;
		return 0;
	...
	#else
	static public bool RaycastToCamera (KiltUICamera p_camera, Vector3 inPos, out RaycastHit2D hit)
	#endif
	{
		#if NGUI_DLL
		UICamera cam = p_camera;
		#else
		KiltUICamera cam = p_camera;
		#endif
		bool v_canUseThisRayCast = (cam is KiltUICamera && ((KiltUICamera)cam).CollisionType.ContainsFlag(ColliderTypeEnum.Raycast2D));
				...
				if (pos.x >=0f && pos.x <= 1f && pos.y >=0f && pos.y <= 1f)
				{
					...
					if (cam.clipRaycasts)
					{
						...
						if (hits.Length > 1)
						{
							...
							//Reverse the List To Make Highest Depths First
	...
	#if NGUI_DLL
	static public bool RaycastToCamera (UICamera p_camera, Vector3 inPos, out RaycastHit hit)
	#else
	static public bool RaycastToCamera (KiltUICamera p_camera, Vector3 inPos, out RaycastHit hit)
	#endif
	{
		#if NGUI_DLL
		UICamera cam = p_camera;
		...
		bool v_canUseThisRayCast = !(cam is KiltUICamera) || (cam is KiltUICamera && ((KiltUICamera)cam).CollisionType.ContainsFlag(ColliderTypeEnum.Raycast3D));
		// Skip inactive scripts
		if (cam != null && cam.enabled && GetActive(cam.gameObject) && v_canUseThisRayCast)
		{
			// Convert to view space
	...
	#if NGUI_DLL
	static public bool RaycastToCamera (UICamera p_camera, Vector3 inPos, out RaycastResult hit)
	#else
	static public bool RaycastToCamera (KiltUICamera p_camera, Vector3 inPos, out RaycastResult hit)
	#endif
	{
	...
	#if NGUI_DLL
	static public new bool Raycast (Vector3 inPos, out RaycastHit hit)
	#else
	...
	#if NGUI_DLL
	//Used to GetWidget of Root or Widget With HighestDepth in TreeRoot (Used to compare who is in front in RayCast);
	static UIWidget GetUIWidgetWithHighestDepthInTransform(Transform p_root)
	{
		UIWidget v_widget = null;
		if(p_root != null)
		{
			v_widget = p_root.GetComponent<UIWidget>();
			if(v_widget == null)
			{
				UIWidget[] v_widgetsInChildrens = p_root.GetComponentsInChildren<UIWidget>();
				foreach(UIWidget v_widgetChild in v_widgetsInChildrens)
				{
					if(v_widget == null)
						v_widget = v_widgetChild;
					else
					{
						if(v_widgetChild.depth > v_widget.depth)
							v_widget = v_widgetChild;
					}
				}
			}
	...
	static DrawTypeEnum GetDrawOrderHighestDepth(Transform p_root, out int p_depth)
	{
		...
		#if NGUI_DLL
		UIWidget v_widget = GetUIWidgetWithHighestDepthInTransform(p_root);
		if(v_renderer != null && v_widget != null)
		{
			v_sucess = v_renderer is MeshRenderer? (DrawTypeEnum.MeshRendererType|DrawTypeEnum.NGUIType) : (v_renderer is SpriteRenderer? (DrawTypeEnum.SpriteRendererType|DrawTypeEnum.NGUIType) : DrawTypeEnum.NGUIType);
			p_depth = v_sucess == DrawTypeEnum.NGUIType? v_widget.depth : Mathf.Max(v_renderer.sortingOrder , v_widget.depth);
		}
		else if(v_sucess == (DrawTypeEnum)0 && v_widget != null)
		{
			v_sucess = DrawTypeEnum.NGUIType;
			p_depth = v_widget.depth;
		}
		#endif
	...
	{
		#if NGUI_DLL
		UIPanel panel = FindInParents<UIPanel>(hit.collider.gameObject);
	...
	#if NGUI_DLL
	static new public UICamera FindCameraForLayer (int layer)
	{
	...
	#else
	static public KiltUICamera FindCameraForLayer (int layer)
	{
		int layerMask = 1 << layer;
	...
	static protected void SafeNotify (GameObject go, string funcName, object obj)
	{
		if (go != null)
		{
			...
			foreach(MonoBehaviour v_behaviour in v_behavioursToSend)
			{
				if(v_behaviour != null)
				{
					KiltUtils.CallFunction(v_behaviour, funcName, obj);
				}
			}
		}
	}
	...
	#else
	static public void AddInCorrectDepthPos(KiltUICamera p_uiCamera)
	{
		//Remove Nulls and Previus added components of this object
		List<KiltUICamera> v_list = new List<KiltUICamera>(UICamList);
		v_list.RemoveChecking(p_uiCamera);
		UICamList = new List<KiltUICamera>(v_list.ToArray());
		//We must add cameras with Highest Depth First
		if(p_uiCamera != null)
		{
			Camera v_camera = p_uiCamera.gameObject.GetComponent<Camera>();
	...
	protected virtual void OnDestroy () { UICamList.Remove(this); }
	
	/// <summary>
	/// Sort the list when enabled.
	/// </summary>
	
	protected virtual void OnEnable () { UICamList.Sort(CompareFunc); }
